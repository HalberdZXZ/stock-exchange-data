...
sequence_lendth = 50; input_dim = 2; batch_size = 32; epoch = 2


model = tf.keras.Sequential([
    tf.keras.layers.LSTM(100, input_shape=(sequence_lendth-1, input_dim),return_sequences=True),
...
# history = model.fit(x_train, y_train, secuenqu_lendth = secuenqu_lendth, input_dim = input_dim, batch_size = batch_size, epoch = epoch)
...
def normalise_networks(window_data, single_window=False): #by_prise_by_first_and_volume_by_middle
    normalised_data = []
...
def next_normalise( i, seq_len, normalise):
    ...
    window = normalise_networks(window, single_window=True)[0] if normalise else window
    x = window[:-1]
    y = window[-1, [0]]
    return x, y   
...
x_train, y_train = get_train_data(seq_len=sequence_lendth, normalise=True)
...
def get_train_data2(seq_len, normalise):
    ...
    data_windows = normalise_networks(data_windows, single_window=False) if normalise else data_windows
...
model.fit(x_train, y_train, epochs=epoch, batch_size=batch_size, callbacks=callbacks)
...
def get_last_data( seq_len, normalise):
    ...
    #data_windows = self.normalise_networks(data_windows, single_window=False) if normalise_networks else data_windows
    data_windows = normalise_networks(data_windows, single_window=True) if normalise else data_windows
    return data_windows
